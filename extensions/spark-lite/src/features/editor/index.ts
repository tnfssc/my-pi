...
import { renderSparkStatusLeft, renderSparkStatusRight } from "../status-line";
...
class Editor extends CustomEditor {
  ...
  render(width: number): string[] {
    ...
    this.removeBottomBorder(lines);

    return lines;
  }

  private removeBottomBorder(lines: string[]): void {
  ...
  private getRight(): string {
    ...
    const statusRight = renderSparkStatusRight(this.ctx, theme);

    return theme.fg("dim", [modelBeforeText, modelText, statusRight].filter(Boolean).join(" • "));
  }
}
