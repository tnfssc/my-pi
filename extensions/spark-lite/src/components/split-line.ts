...
export class SplitLine implements Component {
  ...
  render(width: number): [string] {
    ...
    if (width <= this.padding * 2) return [this.spacingChar.repeat(width)];
  ...
  private renderContent(width: number): string {
    ...
    const secondaryWidth = renderedPrimaryWidth > 0 ? Math.max(0, width - renderedPrimaryWidth - this.gap) : width;
    ...
    const spacingWidth = Math.max(0, width - visibleWidth(renderedLeft) - visibleWidth(renderedRight));
